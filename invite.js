...
/* the closing page names only this page's days, e.g. "27 · 28 NOVEMBER 2026" */
(function closingDates() {
  const el = closeScene && closeScene.querySelector(".place");
  if (!el || !EVENTS.length) return;
  const d = EVENTS.map(e => e.date.split(" "));            // ["26","November","2026"]
  const sameMonth = d.every(x => x[1] === d[0][1] && x[2] === d[0][2]);
  el.innerHTML = sameMonth
    ? d.map(x => x[0]).join(" &middot; ") + " " + (d[0][1] + " " + d[0][2]).toUpperCase()
    : d.map(x => x.join(" ").toUpperCase()).join(" &middot; ");
})();
